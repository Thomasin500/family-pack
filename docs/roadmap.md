...
## Phase 5: Trip Stats & Visualization — DONE

_Built April 19, 2026. This is where the app stopped feeling like a spreadsheet. Pack-class labels had landed earlier in Phase 4.6._

| Feature                         | What                                                                                                                                                                                                                                               | Effort | Status   |
| ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | -------- |
| Trip insights                   | 3–5 actionable one-liners computed from pack contents: heaviest single item, shared-gear imbalance, overloaded pack warnings, empty packs, Ultralight shout-outs. `src/lib/trip-insights.ts` + `computeTripInsights`. **No competitor does this.** | S-M    | **Done** |
| Trip stats panel                | Collapsible drawer (`trip-stats-panel.tsx`) with Insights, Tags, Household Totals, Base/Consumable/Worn stacked bar, Shared Balance, Category charts (bar + pie toggle). Chevron on the left to match every other collapsible.                     | M      | **Done** |
| Shared weight balance           | Stacked bar per person + smart summary ("Thomas is carrying 68% of shared gear"). Inside the stats panel.                                                                                                                                          | S-M    | **Done** |
| Personal vs shared % (per pack) | Inline chip under each pack's totals — "72% personal · 28% shared" with a thin two-color bar. `PersonalSharedChip` in `pack-column.tsx`.                                                                                                           | S      | **Done** |
| Pack class labels (UI)          | Human Ultralight/Lightweight/Traditional/Heavy and pet Trail Runner/Trail Partner/Pack Dog/Overloaded on pack headers; Base Weight tile also color-ramped to match.                                                                                | S      | **Done** |
| Smart trip tags                 | Auto-derived from pack contents (Warm Rated / Cold Weather / Fishing / Multi-Day / Ultralight / Lightweight / Bear Safe / Dog Friendly / Minimalist / Well-Equipped). `src/lib/trip-tags.ts` + `computeTripTags`.                                  | S      | **Done** |
| Trip metadata expansion         | Schema adds `distanceMiles`, `elevationGainFt`, `elevationHighFt`, `durationDays` (all nullable). Edit dialog exposes Distance, Elev Gain, High Point inputs.                                                                                      | S-M    | **Done** |
| Category weight charts          | Horizontal bar chart per pack + per-category color coding. In the stats panel.                                                                                                                                                                     | S      | **Done** |
| Body weight % on trip tile      | Per-person trip tile column shows % body-wt colored by the household's carry tiers, alongside Base and Carry.                                                                                                                                      | XS     | **Done** |
| Totals always in lb             | Every pack total, trip-tile total, stats panel household/shared total, and loadout modal header renders in lb regardless of the nav unit toggle — items still flip with the toggle. Deferred the two-axis split to Phase 11.                       | S      | **Done** |
| 💡 Pie/donut chart toggle       | Pie/donut variant of the category chart, persisted via localStorage. **From a user suggestion.**                                                                                                                                                   | S      | **Done** |
| 💡 Base / Consumable / Worn     | Stacked bar per person inside the stats panel. **From a user suggestion.**                                                                                                                                                                         | S      | **Done** |
| 💡 Base weight over time        | Multi-trip line chart on the dashboard — one line per adult, trend direction detection. `BaseWeightTrend`. **From a user suggestion.**                                                                                                             | M      | **Done** |
| 💡 Distance + elevation trends  | Dual-axis line chart under the base-weight trend (distance mi + elev gain ft). Hidden until ≥ 2 trips have metadata. `TripMetricsTrend` in `base-weight-trend.tsx`. **From a user suggestion.**                                                    | S      | **Done** |
